...
function getProdutos(){
	...
	$select = "select * from curso_php_basico.produtos";
...
function buscarProdutoId($id){
	$conexao = getConexao();
	$select = "select * from produtos where idprodutos=:id";
	$stmt = $conexao->prepare($select);
	$stmt->bindValue(':id',(int)$id);
	$stmt->execute();
	return $stmt->fetch(\PDO::FETCH_ASSOC); //chaves em forma de string
}

function salvarProduto($dados){
	$conexao = getConexao();
	
	$update = "Update produtos set titulo=:titulo, descricao=:descricao, valor=:valor where idprodutos=:id";
...
function deletarProduto($id){
	$conexao = getConexao();
	$delete = "Delete from produtos where idprodutos=:id";
